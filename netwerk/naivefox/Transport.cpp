...
enum class CarrierBody { Stream, Fixed, Document, Style, Script, Image };
 ...
 public:
  ...
  CarrierRequest(size_t aSize, uint32_t aStatus, bool aCell,
    ...
    if (aBodyKind == CarrierBody::Document) {
      mSite = MakeUnique<TransportSite>();
    }
  }

  nsresult Start(const TransportConfig& aConfig, const nsACString& aPath,
    ...
    switch (mBodyKind) {
      ...
      case CarrierBody::Image:
        ...
        break;
      case CarrierBody::Stream:
      case CarrierBody::Fixed:
        break;
    }
    MOZ_TRY(CreateOriginChannel(aConfig.mProxyUrl, aPath, mProtocol,
    ...
    MOZ_TRY(mCookies->RequestHeader(uri, cookie));
    if (!cookie.IsEmpty()) {
    ...
    auto timer = NS_NewTimerWithCallback(
        ...
        nsITimer::TYPE_ONE_SHOT, "NaiveFox::TransportRequestDeadline"_ns);
    if (timer.isErr()) {
  ...
  void Cancel(nsresult aStatus) {
    ...
    if (mChannel) {
      mChannel->Cancel(NS_FAILED(aStatus) ? aStatus : NS_BINDING_ABORTED);
    }
  }

  std::function<void()> mStarted;
  std::function<bool(const Bytes&)> mCellReceived;
  Bytes mBody;
  nsCString mMime;
  nsCString mDigest;
  UniquePtr<TransportSite> mSite;
...
NS_IMETHODIMP CarrierRequest::OnStartRequest(nsIRequest* aRequest) {
  ...
  mResponseProtocol = protocol.EqualsLiteral("h3")   ? "h3"
                      : protocol.EqualsLiteral("h2") ? "h2"
                                                     : "other";
  nsAutoCString trace;
  if (NS_SUCCEEDED(http->GetResponseHeader("CF-Ray"_ns, trace)) &&
      trace.Length() <= 64 &&
      std::all_of(trace.BeginReading(), trace.EndReading(), [](char c) {
  ...
  MOZ_TRY(mCookies->ResponseHeaders(mChannel));
  if (mBodyKind == CarrierBody::Stream) {
    nsAutoCString type;
    MOZ_TRY(mChannel->GetContentType(type));
    if (!type.EqualsLiteral("application/octet-stream")) {
      return NS_ERROR_CORRUPTED_CONTENT;
    }
    MOZ_TRY(mTimer->SetDelay(75000));
    if (mStarted) {
      auto started = std::move(mStarted);
      started();
    }
    return NS_OK;
  }
  if (mExpectedStatus == 200) {
    ...
    if (mBodyKind == CarrierBody::Fixed) {
      if (length >= 0 && length != static_cast<int64_t>(mExpectedSize)) {
        return NS_ERROR_CORRUPTED_CONTENT;
      }
    } else {
      if (mBodyKind == CarrierBody::Document && !length) {
        return NS_ERROR_CORRUPTED_CONTENT;
      }
      mPublicLength = length;
      nsAutoCString type;
      MOZ_TRY(mChannel->GetContentType(type));
      mMime = type;
      const bool valid =
...
NS_IMETHODIMP CarrierRequest::OnDataAvailable(nsIRequest*,
  ...
  if (mBodyKind == CarrierBody::Stream) {
    ...
    while (aCount) {
      ...
      if (!read || !mDecoder.Feed(buffer.data(), read, mCellReceived)) {
        return NS_ERROR_CORRUPTED_CONTENT;
      }
      aCount -= read;
    }
    return mTimer ? mTimer->SetDelay(75000) : NS_ERROR_ABORT;
  }
  if (mBodyKind != CarrierBody::Fixed) {
    if (aCount > UINT64_MAX - mPublicReceived ||
        (mPublicLength >= 0 &&
         aCount > static_cast<uint64_t>(mPublicLength) - mPublicReceived)) {
      return NS_ERROR_CORRUPTED_CONTENT;
    }
    std::array<uint8_t, 16384> buffer;
    while (aCount) {
...
NS_IMETHODIMP CarrierRequest::OnStopRequest(nsIRequest*, nsresult aStatus) {
  ...
  mChannel = nullptr;
  mStarted = nullptr;
  mCellReceived = nullptr;
  if (mBodyKind == CarrierBody::Stream && NS_SUCCEEDED(aStatus)) {
    aStatus =
        mDecoder.Empty() ? NS_ERROR_NET_RESET : NS_ERROR_CORRUPTED_CONTENT;
  }
  if (NS_SUCCEEDED(aStatus)) {
    ...
    } else if ((mPublicLength >= 0 &&
                mPublicReceived != static_cast<uint64_t>(mPublicLength)) ||
               (mBodyKind == CarrierBody::Document && !mPublicReceived)) {
      aStatus = NS_ERROR_CORRUPTED_CONTENT;
    } else if (mSite) {
  ...
  if (NS_FAILED(aStatus)) {
    RuntimeLogEvent(
        "NaiveFox HTTP request failed phase=%u status=0x%08x "
        "http_status=%u protocol=%s trace=%s\n",
        static_cast<unsigned>(mBodyKind), static_cast<unsigned>(aStatus),
        mResponseStatus, mResponseProtocol, mTraceID.get());
  }
  if (mDone) {
...
std::vector<RefPtr<TransportCarrier>> sCarriers;

}  // namespace

class TransportStream::Impl final {
 public:
  Impl(nsIAsyncInputStream* aInput, nsIAsyncOutputStream* aOutput,
       const TransportConfig& aConfig, nsIEventTarget* aTarget,
  ...
  RefPtr<TransportCarrier> carrier;
  UniquePtr<wire::StreamState> state;
  nsCOMPtr<nsITimer> deadline;
  nsCString authority;
  wire::UploadBuffer upload;
  ...
  size_t UploadLimit() const { return wire::kMaxCell; }
};

class TransportCarrier final {
 public:
  NS_INLINE_DECL_THREADSAFE_REFCOUNTING(TransportCarrier)
  explicit TransportCarrier(const TransportConfig& aConfig)
      : mConfig(aConfig) {}
  bool Closed() const { return mClosed; }
  bool CanAttach() const {
    return !mClosed && mStreams.size() < wire::kMaxStreams &&
           mHighestStream != UINT32_MAX;
  }
  bool Matches(const TransportConfig& aConfig) const {
  ...
  void Attach(TransportStream* aStream);
  void Detach(TransportStream* aStream, bool aReset);
  void Wake();
 ...
 private:
  ...
  bool ReceiveCell(const Bytes& aBody, bool aRealtime);
  ...
  bool UploadBlocked() const {
    ...
    if (mHttp3Uploads >= wire::kHttp3ReceiveWindow / 131072) {
      return true;
    }
    bool control = false;
    const bool urgent =
        !mResets.empty() ||
        std::any_of(mStreams.begin(), mStreams.end(), [](const auto& stream) {
          const auto& state = *stream->mImpl;
          return state.openPending ||
                 (state.eof && state.upload.Empty() && !state.state->SentFin());
        });
    // Keep one request slot available for payload and stream lifecycle work.
    return mHttp3Uploads >= 3 && Pressure(control) == 0 && !urgent;
  }
  void SendTick(bool aHeartbeat = false);
  ...
  nsCOMPtr<nsITimer> mReceiveDeadline;
  size_t mPendingMessageBytes = 0;
  uint32_t mAcknowledgedUpload = 0;
  uint32_t mScheduledDelay = 0;
  bool mRealtimeReady = false;
  bool mHeartbeatScheduled = false;
};

NS_IMPL_ISUPPORTS(TransportStream, nsIInputStreamCallback,
                  nsIOutputStreamCallback)

TransportStream::TransportStream(nsIAsyncInputStream* aInput,
                                 nsIAsyncOutputStream* aOutput,
                                 const TransportConfig& aConfig,
...
nsresult TransportStream::Start(const nsACString& aAuthority,
  ...
  return NS_DispatchToMainThread(NS_NewRunnableFunction(
      ...
      [self, authority = std::move(authority), initial = std::move(initial)]() {
        ...
        RefPtr carrier = new TransportCarrier(state.config);
        sCarriers.push_back(carrier);
        carrier->Attach(self);
      }));
}

nsresult TransportStream::StartPump() {
  RefPtr self = this;
  return NS_DispatchToMainThread(
      NS_NewRunnableFunction("NaiveFox::TransportPump", [self]() {
...
void TransportStream::Cancel(nsresult aStatus) {
  mImpl->cancelled = true;
  RefPtr self = this;
  (void)NS_DispatchToMainThread(NS_NewRunnableFunction(
      "NaiveFox::TransportCancel",
      [self, aStatus]() { self->Finish(aStatus, true); }));
}

void TransportStream::Finish(nsresult aStatus, bool aReset) {
  ...
  if (callback) {
    (void)s.target->Dispatch(NS_NewRunnableFunction(
        "NaiveFox::TransportClosed",
        [callback = std::move(callback), aStatus]() { callback(aStatus); }));
  }
}

void TransportStream::ArmRead() {
  auto& s = *mImpl;
...
NS_IMETHODIMP TransportStream::OnInputStreamReady(nsIAsyncInputStream*) {
  MOZ_ASSERT(NS_IsMainThread());
  auto& s = *mImpl;
...
NS_IMETHODIMP TransportStream::OnOutputStreamReady(nsIAsyncOutputStream*) {
  MOZ_ASSERT(NS_IsMainThread());
  Flush();
  return NS_OK;
}

bool TransportCarrier::Open(const nsACString& aPath, const Bytes* aUpload,
  ...
  nsresult rv = request->Start(mConfig, aPath, mCookies, aUpload);
  if (NS_FAILED(rv)) {
    Fail(rv);
    return false;
  }
...
void TransportCarrier::Attach(TransportStream* aStream) {
  MOZ_ASSERT(NS_IsMainThread());
  if (!CanAttach()) {
  ...
  auto timer = NS_NewTimerWithCallback(
      ...
      30000, nsITimer::TYPE_ONE_SHOT, "NaiveFox::TransportOpenDeadline"_ns);
  if (timer.isErr()) {
...
void TransportCarrier::Detach(TransportStream* aStream, bool aReset) {
  auto& s = *aStream->mImpl;
  if (aReset && !mClosed && !s.openPending && s.state) {
    Frame reset;
    if (s.state->MakeReset(reset)) {
      if (mResets.size() >= wire::kMaxStreams) {
...
void TransportCarrier::Fail(nsresult aStatus) {
  if (mClosed) {
    return;
  }
  mClosed = true;
  ...
  mResets.clear();
  RuntimeLogEvent("NaiveFox carrier closed status=0x%08x\n",
                  static_cast<unsigned>(aStatus));
}

void TransportCarrier::Start() {
  mStarted = true;
  mBusy = true;
  RefPtr self = this;
  Open(
...
void TransportCarrier::LoadAssets() {
  if (mClosed) {
    return;
  }
  if (mAssets == mResources.Length()) {
    TransportHash snapshot;
    bool valid =
        snapshot.Field("naivefox-site"_ns) && snapshot.Field(mRootDigest);
...
bool TransportCarrier::Upload(size_t aCapacity, Bytes& aBody) {
  if (mUp == UINT32_MAX) {
    Fail(NS_ERROR_FILE_TOO_BIG);
    return false;
  }
  std::vector<Frame> frames;
  size_t budget = aCapacity - wire::kCellHeader;
  if (!mAuthed) {
    Frame auth;
    auth.kind = Kind::Auth;
    ...
    mAuthed = true;
    if (!wire::Encode(mUp++, aCapacity, frames, aBody)) {
      Fail(NS_ERROR_FAILURE);
      return false;
    }
    return true;
  ...
  while (!mResets.empty() && budget >= wire::kFrameHeader) {
  ...
  for (const auto& stream : mStreams) {
    ...
    if (s.authority.Length() + wire::kFrameHeader > budget) {
      break;
    }
    Frame frame;
  ...
  size_t misses = 0;
  while (!mStreams.empty() && budget >= wire::kFrameHeader &&
         frames.size() < wire::kMaxFrames && misses < mStreams.size()) {
...
bool TransportCarrier::Receive(CarrierRequest* aRequest) {
...
bool TransportCarrier::ReceiveCell(const Bytes& aBody, bool aRealtime) {
  if (mDown == UINT32_MAX) {
    Fail(NS_ERROR_CORRUPTED_CONTENT);
    return false;
  }
  std::vector<Frame> frames;
  if (!wire::Decode(mDown, aBody.size(), aBody, frames)) {
    Fail(NS_ERROR_CORRUPTED_CONTENT);
    return false;
  }
  if (!mContractReady) {
  ...
  for (auto& frame : frames) {
    if (frame.kind == Kind::Ack) {
      if (!aRealtime || frame.stream || !frame.body.empty() ||
          frame.sequence < mAcknowledgedUpload || frame.sequence >= mUp) {
        Fail(NS_ERROR_CORRUPTED_CONTENT);
        return false;
      }
      mAcknowledgedUpload = frame.sequence;
...
void TransportCarrier::ConfirmUpload(uint32_t aSequence) {
  for (const auto& stream : mStreams) {
    auto& s = *stream->mImpl;
...
void TransportCarrier::RetireStreams() {
  auto streams = mStreams;
  for (const auto& stream : streams) {
...
size_t TransportCarrier::Pressure(bool& aControl) const {
  size_t bytes = 0;
...
void TransportCarrier::Wake() {
  ...
  nsresult rv = NS_DispatchToMainThread(
      NS_NewRunnableFunction("NaiveFox::TransportSchedule", [self]() {
        self->mQueued = false;
        if (!self->mClosed) {
          self->Tick();
        }
...
void TransportCarrier::Continue() {
  mBusy = false;
  Wake();
}

void TransportCarrier::Tick() {
  RetireStreams();
  ...
  if (mWebSocket || mHttp3Stream || mPacket) {
    if (mRealtimeReady) {
      ...
      if (bytes || control) {
        ScheduleSend(CoalescingDelay(bytes), false);
      }
    }
    return;
  }
...
void TransportCarrier::StartHttp3() {
  MOZ_ASSERT(mBootstrapped && mStartup == kStartupSlots.size() &&
             mRequests.empty());
  mAcknowledgedUpload = mUp - 1;
  RefPtr self = this;
  mHttp3Stream = new CarrierRequest(
      0, 200, false, ProxyProtocol::H3,
      [self](CarrierRequest*, nsresult aStatus) {
        self->Fail(NS_FAILED(aStatus) ? aStatus : NS_ERROR_NET_RESET);
      },
      CarrierBody::Stream);
  mHttp3Stream->mStarted = [self]() {
    if (self->mClosed) {
      return;
    }
    self->mRealtimeReady = true;
  ...
  nsresult rv =
      mHttp3Stream->Start(mConfig, "/api/stream"_ns, mCookies, nullptr);
  if (NS_FAILED(rv)) {
    Fail(rv);
  }
}

void TransportCarrier::StartWebSocket() {
  MOZ_ASSERT(mBootstrapped && mAssets == mResources.Length() &&
             mStartup == kStartupSlots.size() && mRequests.empty());
  mAcknowledgedUpload = mUp - 1;
  RefPtr self = this;
  mWebSocket = new TransportWebSocket(
      [self]() {
        if (self->mClosed) {
          return;
        }
        ...
        self->mReceiveDeadline = deadline.unwrap();
      ...
      [self](const nsACString& aMessage) {
        ...
        const bool valid = wire::ValidRealtimeDownCapacity(length);
        if (!valid) {
          self->Fail(NS_ERROR_CORRUPTED_CONTENT);
          return;
        }
        ...
        if (self->ReceiveCell(Bytes(data, data + length), true)) {
          if (self->mReceiveDeadline) {
            nsresult rv = self->mReceiveDeadline->SetDelay(75000);
            if (NS_FAILED(rv)) {
              self->Fail(rv);
              return;
            }
      ...
      [self](uint32_t aSize) {
        ...
        if (aSize > self->mPendingMessageBytes) {
          self->Fail(NS_ERROR_UNEXPECTED);
          return;
        }
        self->mPendingMessageBytes -= aSize;
        if (!self->mPendingMessageBytes) {
          bool control = false;
  ...
  nsresult rv =
      mWebSocket->Start(mConfig, mCookies, "/api/realtime"_ns, "naivefox"_ns);
  if (NS_FAILED(rv)) {
    Fail(rv);
  }
}

bool TransportCarrier::HasPendingOpen() const {
...
void TransportCarrier::ScheduleSend(uint32_t aDelay, bool aHeartbeat) {
  if (mClosed || !mRealtimeReady || UploadBlocked()) {
    return;
  }
  if (mSendTimer) {
    if (aHeartbeat || (!mHeartbeatScheduled && aDelay >= mScheduledDelay)) {
      return;
    }
    mSendTimer->Cancel();
    mSendTimer = nullptr;
  }
  mHeartbeatScheduled = aHeartbeat;
  mScheduledDelay = aDelay;
  RefPtr self = this;
  auto timer = NS_NewTimerWithCallback(
      [self, aHeartbeat](nsITimer*) {
        self->mSendTimer = nullptr;
        self->SendTick(aHeartbeat);
      },
  ...
  mSendTimer = timer.unwrap();
}

void TransportCarrier::SendTick(bool aHeartbeat) {
  if (mClosed || !mRealtimeReady || UploadBlocked()) {
    return;
  }
  bool control = false;
  ...
  if (mConfig.mProtocol == ProxyProtocol::H3) {
    ++mHttp3Uploads;
    RefPtr self = this;
    if (!Open("/api/upload"_ns, &body, 0, 204, false,
