...
| `NF-UPSTREAM-018` | Use an isolated NaiveFox Rust workspace and product ping root rather than Firefox-wide incidental feature ownership; explicitly link `mozglue-static` and propagate `MOZ_MEMORY` so Rust/C++ share GeckoAlloc. | root and toolkit Rust manifests, `config/makefiles/rust.mk`, mozbuild emitter, Glean parser, `mozglue/static/rust` allocator | Deterministic metadata on all targets; clean standalone build; closure report consistency; staged `--runtime-smoke` nested-array ownership probe; live Windows H3. |
