...
case $naivefox_arm in
  document-first-buffer-task-optimistic | document-first-buffer-http-connect-optimistic | document-first-buffer-task-early-data | document-first-buffer-http-connect-early-data) ;;
...
if [[ ( $naivefox_arm == document-first-buffer-task-early-data ||
        $naivefox_arm == document-first-buffer-http-connect-early-data ) &&
...
if [[ $experiment_design == multi_arm_superblocks ]]; then
  ...
  for arm in "${multi_arm_arms[@]}"; do
    ...
    if [[ ( $arm == document-first-buffer-task-early-data ||
            $arm == document-first-buffer-http-connect-early-data ) &&
          $protocol_selection != h2 ]]; then
...
run_naivefox_sample() {
  ...
  if [[ $arm == tree-root-overlap || $arm == tree-root-overlap-css ||
    ...
    drain_pattern=" preamble native-parser-preload drain=complete completed_resources=1 http=2[0-9][0-9] protocol=$protocol$"
          ...
          $arm == document-first-buffer-task-early-data ||
...
if [[ $experiment_design == multi_arm_superblocks ||
  ...
  if [[ $naivefox_arm == tree-complete ||
        ...
        $naivefox_arm == document-first-buffer-task-early-data ||
