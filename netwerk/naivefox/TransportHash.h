...
#ifndef netwerk_naivefox_TransportHash_h
#define netwerk_naivefox_TransportHash_h
...
class TransportHash final {
 public:
  TransportHash() : mContext(HASH_Create(HASH_AlgSHA256)) {
    if (mContext) {
      HASH_Begin(mContext);
    }
  }
  ...
  TransportHash(const TransportHash&) = delete;
  TransportHash& operator=(const TransportHash&) = delete;
