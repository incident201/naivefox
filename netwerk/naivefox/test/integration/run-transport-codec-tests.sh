...
objdir=$(cd "$1" && pwd)
output=${2:-"$objdir/transport-codec-tests"}
mkdir -p "$output"
...
if ! env -u LD_PRELOAD -u LD_LIBRARY_PATH "$compiler" "${flags[@]}" \
    ...
    "$source_root/netwerk/naivefox/TransportCodec.cpp" \
    "$source_root/netwerk/naivefox/test/gtest/TestTransportCodec.cpp" \
    "$gtest/src/gtest-all.cc" "$gtest/src/gtest_main.cc" \
    -L"$runtime" -Wl,-rpath-link,"$runtime" -lnss3 -lnssutil3 -lnspr4 \
    -o "$output/transport-codec-tests" >"$output/build.log" 2>&1; then
  printf 'Codec compile failed; see %s/build.log\n' "$output" >&2
  exit 1
fi

if ! env -u LD_PRELOAD LD_LIBRARY_PATH="$runtime" \
    "$output/transport-codec-tests" \
    --gtest_filter="NaiveFoxTransportCodec.*" \
