...
NoConnectSite::~NoConnectSite() = default;

nsresult NoConnectSite::Feed(Span<const uint8_t> aBytes, bool aLast) {
...
nsTArray<SiteResource> NoConnectSite::TakeResources() {
  MOZ_ASSERT(mFinished && NS_SUCCEEDED(mStatus));
  return std::move(mResources);
}

nsresult NoConnectSite::OnElement(int32_t aNamespace, nsAtom* aName,
...
nsresult NoConnectSite::Add(const nsACString& aReference,
