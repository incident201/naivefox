...
SPEC = importlib.util.spec_from_file_location(
    "matched_app_matrix", HERE / "run-matched-app-matrix.py"
)
...
def successful_records():
    ...
    browser = {
        "manifest_sha256": M.MANIFEST_SHA,
        "uploaded_bytes": up,
        "downloaded_bytes": down,
        "assets": list(assets.values()),
        "app_sha256": assets["/assets/app.js"]["sha256"],
        "http": {
            "opened": 1,
            "closed": 1,
            "requests": 13,
    ...
    for spec in manifest["stages"]:
        ...
        received = sum(
            item["bytes"] for item in jobs if item["kind"] in ("download", "echo")
        )
        browser["stages"].append({
            "name": spec["name"],
            "jobs": jobs,
            "io_start_ms": start,
            "io_end_ms": max(item["io_end_ms"] for item in jobs),
            "verified_ms": max(item["verified_ms"] for item in jobs),
            "sent_bytes": sent,
            "received_bytes": received,
            "useful_bytes": sum(item["bytes"] for item in jobs),
        })
    backend_jobs = []
    for job in manifest["jobs"]:
        ...
        value.update(
            verified=True,
            validated=job["bytes"] if job["kind"] in ("upload", "echo") else 0,
    ...
    connection = {
        ...
        "peak_jobs": 4,
        "data_bytes_in": up,
        "expected_data_bytes_in": up,
        "data_bytes_out": down,
        "expected_data_bytes_out": down,
        "data_messages_in": 21,
        "data_messages_out": 165,
        "control_messages_in": 190,
        "control_messages_out": 43,
        "open_order": list(range(1, 12)),
        "asset_cookie_hash": "app",
        "jobs": backend_jobs,
    }
    backend = {
        "manifest_sha256": M.MANIFEST_SHA,
    ...
    for index in range(40):
        backend["api"].append({
            "method": "POST" if index % 2 == 0 else "GET",
            "path": f"/app/api/bootstrap/{index // 2}",
            "asset_cookie_hash": "app",
            "request_bytes": 100 if index % 2 == 0 else 0,
            "request_sha256": "a" * 64,
            "response_bytes": 500,
...
class MatchedApplicationTests(unittest.TestCase):
    def test_navigation_waits_for_load_and_does_not_retry_script_failures(self):
        ...
        self.assertEqual(
            [call[0] for call in driver.method_calls],
            [
        ...
        self.assertEqual(
            driver.set_page_load_timeout.call_args_list, [mock.call(30), mock.call(120)]
        )
        self.assertEqual(
            driver.get.call_args_list,
            [mock.call("https://127.0.0.1:443/health"), mock.call("about:blank")],
        )
        driver.reset_mock()
    ...
    def test_wrong_bytes_hashes_parallelism_and_native_http_fail(self):
        mutations = (
            ...
            lambda b, s: b["http"].update(requests=12),
            lambda b, s: b["stages"][0]["jobs"][0].update(next_hop_protocol="http/1.1"),
    ...
    def test_icmp_counts_outer_packet_only_and_reverses_quoted_direction(self):
        ...
        self.assertEqual(
            (feedback["wire_size"], feedback["direction"], feedback["flow"]),
            (112, 1, ""),
        )
        original = {"wire_size": 84, "direction": -1, "flow": "quic:0"}
        summary = M.origin_wire_summary([original, feedback])
        self.assertEqual(
            (summary["wire_bytes"], summary["packets"], summary["outer_flows"]),
            (196, 2, 1),
        )
        reverse = dict(
            base,
    ...
    def test_active_http_cannot_bypass_selected_native_listener(self):
        def owners(port):
        ...
        with mock.patch.object(
            M, "established_owners", side_effect=bypass
        ), self.assertRaisesRegex(RuntimeError, "bypassed"):
            M.validate_routing_owners(False, 443, 444, 445, 100, {200, 201})
        with mock.patch.object(M, "established_owners", return_value=[{200}]):
            self.assertTrue(
                M.validate_routing_owners(True, 443, 444, 0, 100, {200})["verified"]
            )
    ...
    def test_complete_tcp_fin_payload_wrap_and_retransmission(self):
        def packet(frame, incoming=False, syn=False, fin=False, seq=0, ack=0, length=0):
            return {
                "tcp.stream": "0",
                "tcp.dstport": "42000" if incoming else "443",
                "tcp.srcport": "443" if incoming else "42000",
                "tcp.flags.syn": str(int(syn)),
                "tcp.flags.ack": str(int(not syn)),
                "tcp.flags.fin": str(int(fin)),
                "tcp.flags.reset": "0",
                "tcp.seq_raw": str(seq),
                "tcp.ack_raw": str(ack),
                "tcp.len": str(length),
        ...
        rows = [
            packet(1, syn=True),
            packet(2, fin=True, seq=0xFFFFFFFE, length=2),
    ...
    def test_nonce_cannot_hide_packet_arriving_after_producer_exit(self):
        ...
        link.validate.return_value = {
            "profile": "rtt40-20mbps",
            "qdiscs": [
                {"kind": "netem", "backlog": 0, "qlen": 0},
                {"kind": "netem", "backlog": 0, "qlen": 0},
            ],
        }
        with mock.patch.object(M, "time", clock), mock.patch.object(
            M, "live_owned", return_value=[]
        ), mock.patch.object(
            M, "latest_outer_packet", side_effect=lambda *args: last[0]
        ), mock.patch.object(M, "validate_tcp_termination"), mock.patch.object(
            M, "write_json"
        ):
    ...
    def test_environment_marker_cannot_authorize_host_qdisc_changes(self):
        with mock.patch.dict(
            M.os.environ, {"NAIVEFOX_CAPTURE_ISOLATED_NETWORK_ENTERED": "1"}
        ), mock.patch.object(
            M.os, "readlink", return_value="net:[same]"
        ), mock.patch.object(M, "run_quiet") as commands:
        ...
        with mock.patch.object(
            M, "assert_isolated_namespace", return_value="net:[different]"
        ), mock.patch.object(M, "run_quiet") as commands:
...
class LocalCaddyProofTests(unittest.TestCase):
    ...
    def test_external_application_is_complete(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                len([path for path in root.rglob("*") if path.is_file()]), 7
            )
            self.assertLessEqual((root / "index.html").stat().st_size, 4096)
            for index in range(1, 5):
                self.assertIn(
                    f"/assets/image-{index}.svg", (root / "index.html").read_text()
                )
            self.assertEqual(
                (root / "assets/app.js").read_bytes(), (M.APP / "app.js").read_bytes()
            )
