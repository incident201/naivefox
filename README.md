...
- `preamble` controls only `classic`; `no-connect` uses its own bounded
  ...
  in [`FRONTING-PAGE.md`](netwerk/naivefox/FRONTING-PAGE.md). A change to that contract is not
  ...
  [Current implicit-default matrix](netwerk/naivefox/CAPTURE.md#current-implicit-default-matrix);
  ...
  screens; the subsequent [outer-resource size campaign](netwerk/naivefox/CAPTURE.md) exercises
...
[NO-CONNECT.md](netwerk/naivefox/NO-CONNECT.md). Historical full-browser measurements are not
