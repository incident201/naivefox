...
case $naivefox_arm in
  document-first-buffer-task-optimistic | document-first-buffer-http-connect-optimistic | document-first-buffer-task-delayed-padding | document-first-buffer-http-connect-delayed-padding) ;;
...
if [[ ( $naivefox_arm == document-first-buffer-task-delayed-padding ||
        $naivefox_arm == document-first-buffer-http-connect-delayed-padding ) &&
...
if [[ $experiment_design == multi_arm_superblocks ]]; then
  ...
  if [[ ( -n ${seen_multi_arms[document-first-buffer-task-delayed-padding]:-} ||
          -n ${seen_multi_arms[document-first-buffer-http-connect-delayed-padding]:-} ) &&
        $protocol_selection != h2 ]]; then
    printf 'delayed padding multi-arm screening requires --protocol h2\n' >&2
    exit 2
  fi
...
run_naivefox_sample() {
  ...
  if [[ $arm == document-first-buffer-http-connect ||
        $arm == document-first-buffer-http-connect-delayed-padding ||
  ...
  if [[ $arm == tree-root-overlap || $arm == tree-root-overlap-css ||
    ...
    drain_pattern=" preamble native-parser-preload drain=complete completed_resources=1 http=2[0-9][0-9] protocol=$protocol$"
          ...
          $arm == document-first-buffer-task-delayed-padding ||
...
if [[ $experiment_design == multi_arm_superblocks ||
  ...
  if [[ $naivefox_arm == tree-complete ||
        ...
        $naivefox_arm == document-first-buffer-task-delayed-padding ||
