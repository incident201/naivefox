...
spec = importlib.util.spec_from_file_location(
    "transport_android_fixture_tested",
    Path(__file__).with_name("run-android-runtime-tests.py"),
)
...
class AndroidFixtureTests(unittest.TestCase):
    def fixture(self, work):
        ...
        fixture.remote = "/data/local/tmp/naivefox-native-owned"
    ...
    def test_new_listeners_replace_host_ports_with_one_guest_allocation(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(
                fixture, "capture_command", return_value=completed
            ) as capture, mock.patch.object(
                android.suite,
                "free_port",
                side_effect=AssertionError("host allocation forbidden"),
            ):
    ...
    def test_shared_config_reuses_actual_guest_ports_without_reallocation(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(
                fixture,
                "capture_command",
                side_effect=AssertionError("shared ports changed"),
            ):
                for _ in range(4):
                    actual = dict(ports)
                    fixture.choose_listener_ports(
                        SimpleNamespace(listener_ports=ports), Path(directory), actual
                    )
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(
                ...
                with self.assertRaisesRegex(RuntimeError, "still owned"):
                    fixture.choose_listener_ports(
                        SimpleNamespace(), Path(directory), {"socks": 1, "http": 2}
                    )
            self.assertEqual(capture.call_count, 1)
            ...
            with self.subTest(
                ...
                with mock.patch.object(
                    ...
                    with self.assertRaises(RuntimeError):
                        fixture.choose_listener_ports(
                            SimpleNamespace(), Path(directory), ports
                        )
    ...
    def test_shared_port_drift_and_duplicate_pair_fail(self):
        with tempfile.TemporaryDirectory() as directory:
            fixture = self.fixture(Path(directory))
            for fixed, proposed in (
                ({"socks": 41000, "http": 41001}, {"socks": 41002, "http": 41001}),
                ({"socks": 41000, "http": 41000}, {"socks": 41000, "http": 41000}),
            ):
                with self.subTest(fixed=fixed), self.assertRaises(RuntimeError):
                    fixture.choose_listener_ports(
                        SimpleNamespace(listener_ports=fixed), Path(directory), proposed
                    )
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(android.subprocess, "run", side_effect=run):
                fixture.capture_failure(
                    work, fixture.remote + "/client-owned", "startup", 1, RuntimeError()
                )
    ...
    def test_failure_without_pid_does_not_dump_other_process_logs(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(
                android.subprocess,
                "run",
                return_value=subprocess.CompletedProcess([], 1, b"missing"),
            ) as run:
                fixture.capture_failure(
                    work, fixture.remote + "/client-owned", "startup", 137
                )
            self.assertFalse(
                any("logcat" in call.args[0] for call in run.call_args_list)
            )
            details = json.loads(
                (work / "diagnostics-startup/summary.json").read_text()
            )
    ...
    def test_startup_failure_is_captured_before_remote_cleanup_without_retry(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(
                android.suite, "Process", FailedProcess
            ), mock.patch.object(
                android.suite, "wait_until", side_effect=RuntimeError("startup failed")
            ), mock.patch.object(
                fixture, "capture_command", return_value=allocated
            ), mock.patch.object(fixture, "call", side_effect=call), mock.patch.object(
                fixture,
                "capture_failure",
                side_effect=lambda *args: events.append("capture-" + args[2]),
            ):
            ...
            self.assertLess(
                events.index("capture-startup"), events.index("remote-cleanup")
            )
            self.assertFalse(fixture.ports)

    def test_embedded_api_uses_frozen_config_without_selector(self):
        with tempfile.TemporaryDirectory() as directory:
            work = Path(directory)
            fixture = self.fixture(work)
            launches = []
            ...
            with mock.patch.object(
                android.suite, "Process", ReadyProcess
            ), mock.patch.object(android.suite, "wait_until"), mock.patch.object(
                fixture, "call"
            ) as call:
                original = None
                for _ in range(2):
                    args = SimpleNamespace(
                        ...
                        preserve_client_config=True,
                    )
                    process, _ = fixture.start(
                        args, work, {"listen": []}, {}, dict(ports)
                    )
                    process.stop()
                    if original is None:
                    ...
                    self.assertNotIn("--reject-first", argv)
                pushes = [
                    item.args for item in call.call_args_list if item.args[0] == "push"
                ]
    ...
    def test_frozen_config_drift_is_rejected_before_launch(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            original = b'{"listen": [], "max-connections": 0}\n'
            ...
            with mock.patch.object(fixture, "call"), mock.patch.object(
                android.suite, "Process"
            ) as process:
                with self.assertRaisesRegex(RuntimeError, "shared configuration"):
                    fixture.start(
                        args,
                        work,
                        {"listen": [], "max-connections": 1},
                        {},
