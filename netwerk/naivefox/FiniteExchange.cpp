...
const char* FiniteVersion(const TunnelConfig& aConfig) {
  return aConfig.mDiagnosticH2FiniteBudgetedDownloads
             ? "3"
             : (aConfig.mDiagnosticH2FiniteStreamUploads ? "2" : "1");
}
...
class FiniteExchange::Impl final {
 public:
  ...
  uint64_t mFullDownloadBodies = 0;
...
nsresult FiniteExchange::Open(const char* aOperation, uint64_t aSequence,
  ...
  add("X-Naivefox-Finite", nsDependentCString(FiniteVersion(mImpl->mConfig)));
...
NS_IMETHODIMP FiniteExchange::Listener::OnStartRequest(nsIRequest* aRequest) {
  ...
  if (!protocol.EqualsLiteral("h2") ||
      !marker.Equals(FiniteVersion(mOwner->mImpl->mConfig)) ||
...
void FiniteExchange::Finished(Listener* aListener, nsresult aStatus) {
  ...
  if (aListener->mOperation.EqualsLiteral("down")) {
    ...
    if (aListener->mBody.Length() == kExchangeBytes) {
      ++mImpl->mFullDownloadBodies;
      if (mImpl->mConfig.mDiagnosticH2FiniteBudgetedDownloads &&
          mImpl->mFullDownloadBodies == 1) {
        RuntimeLogEvent(
            "Connection %llu finite-exchanges budgeted-download-complete=1 "
            "bytes=%u\n",
            static_cast<unsigned long long>(mImpl->mConnectionId),
            kExchangeBytes);
      }
...
void FiniteExchange::Cancel(nsresult aStatus) {
  ...
  RuntimeLogEvent(
      ...
      "uploads-started=%llu downloads-started=%llu full-download-bodies=%llu\n",
      static_cast<unsigned long long>(mImpl->mConnectionId),
      ...
      static_cast<unsigned long long>(mImpl->mNextUpload),
      static_cast<unsigned long long>(mImpl->mNextDownload),
      static_cast<unsigned long long>(mImpl->mFullDownloadBodies));
  auto callback = std::move(mImpl->mCallback);
  if (callback) {
