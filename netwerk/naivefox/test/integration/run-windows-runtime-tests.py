...
def fixture_module():
    ...
    spec = importlib.util.spec_from_file_location("transport_fixture", path)
...
def worker(path):
    ...
    if job["action"] == "call":
        allowed = {
            ...
            "echo_wake",
            "cancel_stream",
            "response_first_transfer",
            "open_tunnel",
            "concurrent_open_streams",
            "auth_partition_streams",
            "reject_policy",
        }
    ...
    profile.mkdir()
    env.update(
        PATH=str(runtime.parent) + os.pathsep + env.get("PATH", ""),
        NAIVEFOX_PROFILE=str(profile),
    ...
    try:
        ...
        def ready():
            ...
            return (
                f"SOCKS5 listening on 127.0.0.1:{ports['socks']}" in text
                and f"HTTP CONNECT listening on 127.0.0.1:{ports['http']}" in text
            )

        module.wait_until(
            ready, "native Windows listeners did not start", process, timeout=30
        )
                ...
                for event, _ in poll.select():
                    if event.fileobj is front:
                        ...
                        if peer not in peers:
                            if len(peers) >= 64:
                                raise RuntimeError(
                                    "Windows fixture UDP peer bound exceeded"
                                )
...
def relay_protocols(protocol):
    # Keep TCP reachable for the strict-H3 fallback canary.
    return ("h3", "h2") if protocol == "h3" else ("h2",)
        ...
        while time.monotonic() < deadline:
            for _, _ in poll.select(min(0.1, max(0, deadline - time.monotonic()))):
                ...
                if len(pending) > 256 or (b"\n" in pending and pending != b"ready\n"):
                    raise RuntimeError(
                        "Windows loopback relay returned invalid readiness"
                    )
                if pending == b"ready\n":
                    return
...
def run_inside(args):
    ...
    run = Path(tempfile.mkdtemp(prefix="naivefox-windows-", dir=root))
    ...
    def client_factory(inputs, directory, config, env, ports):
        job = {
            "action": "client",
            "directory": winpath(directory),
            "runtime": winpath(args.runtime),
            "config": config,
        ...
        try:
            module.wait_until(
                lambda: (directory / "ready.json").exists(),
                "native Windows helper did not become ready",
                client,
                timeout=40,
            )
    ...
    def call(name, *positional, **keywords):
        ...
        module.require(
            result == 0, f"native Windows {name} failed; inspect {label}.log"
        )
    ...
    def start_caddy(*arguments):
        ...
        try:
            for carrier_protocol in relay_protocols(protocol):
    ...
    try:
        ...
        for expected in ("http.handlers.naivefox_transport",):
            module.require(
