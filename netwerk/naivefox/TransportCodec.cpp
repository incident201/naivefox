...
bool CellStreamDecoder::Feed(
  ...
  while (aLength) {
    ...
    if (!ValidRealtimeDownCapacity(mLength)) {
      return false;
    }
    const size_t count = std::min(aLength, size_t(mLength) - mBody.size());
    mBody.insert(mBody.end(), aData, aData + count);
    aData += count;
    aLength -= count;
    if (mBody.size() == mLength) {
      if (!aCell(mBody)) {
        return false;
      }
      mBody.clear();
...
bool Decode(uint32_t aExpectedSequence, size_t aExpectedCapacity,
  ...
  if (aExpectedCapacity < kCellHeader || aExpectedCapacity > kMaxCell ||
      ...
      std::memcmp(aInput.data(), "NFOX", 4) != 0 ||
...
bool StreamState::Receive(const Frame& aFrame) {
  ...
  switch (aFrame.kind) {
    ...
    case Kind::Credit: {
      ...
      if (credit == 0 || credit > mWindow - mSendCredit) {
        return Fail();
      }
      mSendCredit += credit;
      return true;
