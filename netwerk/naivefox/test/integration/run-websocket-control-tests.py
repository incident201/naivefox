#!/usr/bin/env python3
"""Verify native control PING/PONG does not consume the NFOX writer budget."""
...
spec = importlib.util.spec_from_file_location(
    "control_fixture", HERE / "run-websocket-adversarial-tests.py"
)
...
def encode(sequence, frames):
    payload = b"".join(
        struct.pack("!B3xIII", kind, stream, offset, len(body)) + body
    ...
    return (
        b"NFOX"
        + struct.pack("!IIHBB", sequence, used, len(frames), 0, 0)
        + payload
        + bytes(512 - used)
    )
        ...
        try:
            ...
            def consume():
                ...
                if opcode == 10:
                    return body
                fixture.require(
                    opcode == (0 if fragment else 2),
                    "unexpected client close or message",
                )
                ...
                fixture.require(
                    body[:4] == b"NFOX" and sequence == up and 16 <= used <= len(body),
                ...
                for _ in range(count):
                    kind, stream, position, size = struct.unpack_from(
                        "!B3xIII", body, offset
                    )
                    value = body[offset + 16 : offset + 16 + size]
                    fixture.require(
                        len(value) == size and stream == 1, "invalid logical frame"
                    )
                    offset += 16 + size
                    if kind == 2:
                        fixture.require(
                            position == 1 and value == b"after-controls",
                            "post-control data differs",
                        )
                        ...
                        fixture.require(
                            position == 1 + len(b"after-controls") and not value,
                            "FIN differs",
                        )
            ...
            for length in (4, 125):
                ...
                self.request.sendall(
                    adversarial.frame(
                        2, encode(down, [(2, 1, output_offset, b"alive")])
                    )
                )
                down += 1
                output_offset += 5
...
def run(args, protocol, listener):
    root = args.work_dir / (protocol + "-" + listener)
    ...
    try:
        ...
        caddy, port = fixture.start_caddy(
            args, root, protocol, target.server_address[1], user, password
        )
        processes.append(caddy)
        client, ports = fixture.start_client(
            args, root, "client", protocol, port, user, password, 1
        )
        processes.append(client)
        with fixture.open_tunnel(ports, listener, target.server_address[1]) as local:
            local.sendall(b"E")
            fixture.require(
                fixture.receive(local, 10) == b"alivealive",
                "control PING aborted the carrier",
            )
            local.sendall(b"after-controls")
            fixture.require(
                fixture.receive(local, len(b"after-controls")) == b"after-controls",
                "echo failed after controls",
            )
            local.shutdown(socket.SHUT_WR)
            fixture.require(
                fixture.receive(local, 4) == b"tail", "final payload differs"
            )
            fixture.require(
                not local.recv(1), "half-close failed without a CREDIT acknowledgement"
            )
        client.exited_cleanly()
        fixture.require(
            peer.finished.wait(8) and not peer.failures,
            "control peer failed: " + str(peer.failures),
        )
        fixture.require(
            peer.pongs == 2 and peer.data_verified,
            "missing legal controls or post-control data",
        )
        result = {
            "protocol": protocol,
            "listener": listener,
            "status": "PASS",
            "pong_payload_bytes": [4, 125],
            "data_verified": True,
            "half_close": True,
        }
...
def main():
    ...
    fixture.require(
        args.work_dir.resolve().is_relative_to(args.objdir.resolve())
        and not args.work_dir.exists(),
    ...
    for protocol in ("h2",):
        for listener in (
            ...
            results.append(run(args, protocol, listener))
