...
def fixture_identity(root):
    ...
    resources = [
        ("/assets/site.css", "style", "text/css"),
        ("/assets/app.js", "script", "text/javascript"),
    ]
    resources += [
        (f"/assets/image-{i}.svg", "image", "image/svg+xml") for i in range(1, 5)
    ]
...
def mutation(case):
    def apply(server):
        if case == "protocol":
            return
        handlers = [
            item for route in server["routes"] for item in route.get("handle", [])
        ]
        root = None
        while handlers:
            item = handlers.pop()
        ...
        profile = "incompatible" if case == "hello-contract" else "naivefox"
        ...
        frame = (
            struct.pack(
                "!B3xIII",
                9,
                int(case == "hello-stream"),
                int(case == "hello-sequence"),
                len(payload),
            )
            + payload
        )
        ...
        header = bytearray(
            b"NFOX"
            + struct.pack("!IIHH", int(case == "sequence"), 16 + len(frame), count, 0)
        )
        if case == "reserved":
            header[14] = 1
        ...
        if case in ("cookie-missing", "cookie-malformed"):
            ...
            if case == "cookie-malformed":
                response["headers"]["Set-Cookie"] = [
                    "session=invalid; Path=/; Secure; HttpOnly"
                ]
...
def run_case(args, base, protocol, case):
    ...
    try:
        ...
        client_protocol = (
            ("h3" if protocol == "h2" else "h2") if case == "protocol" else protocol
        )
        client, ports = fixture.start_client(
            args, run, "client", client_protocol, port, user, password, 1
        )
        ...
        if case in (
            ...
            fixture.require(
                posts == 0, "public bootstrap rejection sent authentication"
            )
        else:
        ...
        if case == "auth-prompt":
            fixture.require(
                sum(item.get("uri") == "/" for item in requests) == 1,
        ...
        if case == "truncated":
            ...
            fixture.require(
                bool(broken)
                and broken[-1]["size"] == 100
                and broken[-1].get("resp_headers", {}).get("Content-Length")
                == ["8192"],
...
def main():
    parser = argparse.ArgumentParser(
        description="Fail-closed native NaiveFox HTTP envelope and protocol checks."
    )
    ...
    run = Path(tempfile.mkdtemp(prefix="transport-adversarial-", dir=root))
    try:
        results = [
            run_case(args, run, protocol, case)
            for protocol in (
