...
spec = importlib.util.spec_from_file_location(
    "transport_runtime", Path(__file__).with_name("transport_fixture.py")
)
...
def cell(sequence=20, capacity=512, ack=None, stream=0, body=b"", hint=0):
    frames = (
        b"" if ack is None else struct.pack("!B3xIII", 8, stream, ack, len(body)) + body
    )
    used = 16 + len(frames)
    fixture.require(used <= capacity, "invalid adversarial fixture capacity")
    return (
        b"NFOX"
        + struct.pack("!IIHBB", sequence, used, int(ack is not None), hint, 0)
        + frames
        + bytes(capacity - used)
    )
...
def read_client_frame(sock):
    ...
    fixture.require(
        first & 0x70 == 0 and second & 0x80, "client sent invalid WebSocket frame"
    )
    size = second & 0x7F
    if size == 126:
...
def receive_client_cell(sock):
    ...
    while True:
        ...
        fixture.require(
            opcode == (0 if started else 2),
            "expected client binary application message",
        )
        started = True
        body.extend(part)
        fixture.require(
            len(body) <= 262144, "client fragmented message exceeded fixture bound"
        )
        if final:
            break
    capacities = (512, 4096, 16384, 131072)
    fixture.require(
        len(body) in capacities
        and body[:4] == b"NFOX"
        and body[14] == 0
        and body[15] == 0,
        "client did not send a shaped NFOX message",
    )
    ...
    def handle(self):
        ...
        try:
            ...
            while not header.endswith(b"\r\n\r\n"):
                ...
                fixture.require(
                    len(header) <= 16384, "WebSocket handshake exceeded fixture bound"
                )
            lines = header.decode("ascii").split("\r\n")
            fixture.require(
                lines[0] == "GET /api/realtime HTTP/1.1",
                "unexpected upstream WebSocket request",
            )
            headers = {}
            for line in lines[1:]:
                if line:
            ...
            fixture.require(
                ...
                and headers.get("sec-websocket-protocol") == server.protocol
                and headers.get("sec-websocket-version") == "13",
                "missing genuine WebSocket handshake",
            )
            fixture.require(
                "authorization" not in headers and "proxy-authorization" not in headers,
                "proxy credentials leaked into WebSocket headers",
            )
            fixture.require(
                headers.get("cookie", "").startswith("session="),
                "session cookie missing",
            )
            key = headers.get("sec-websocket-key", "")
            fixture.require(
                len(base64.b64decode(key, validate=True)) == 16,
                "invalid WebSocket handshake key",
            )
            ...
            if server.case != "missing-subprotocol":
                reply.append(
                    ...
                    + (
                        "wrong.protocol"
                        if server.case == "wrong-subprotocol"
                        else server.protocol
                    )
                )
            if server.case == "unsolicited-compression":
                reply.append("Sec-WebSocket-Extensions: permessage-deflate")
            fixture.require(
                server.allow_response.wait(10),
                "local stream did not reach the attack gate",
            )
            self.request.sendall(("\r\n".join(reply) + "\r\n\r\n").encode("ascii"))
            server.upgraded.set()
            if server.case not in HANDSHAKE_CASES:
                upstream = (
                    receive_client_cell(self.request)
                    if server.case in ACK_CASES
                    else 20
                )
                body = cell()
                opcode = 2
...
def run_case(args, base, protocol, case, listener):
    ...
    try:
        ...
        client, ports = fixture.start_client(
            args, run, "client", protocol, port, user, password, 1
        )
        ...
        with fixture.open_tunnel(ports, listener, target.server_address[1]) as local:
            ...
            fixture.require(
                malicious.upgraded.wait(10),
                "client never reached the WebSocket handshake",
            )
            ...
            try:
                fixture.require(
                    not local.recv(1), "malformed WebSocket delivered proxy payload"
                )
            except ConnectionResetError:
                pass
            closed_at = time.monotonic()
        fixture.require(
            malicious.finished.wait(6), "malicious responder did not observe shutdown"
        )
        fixture.require(
            not malicious.failures and malicious.closed_by_client,
            "client did not reject the malformed WebSocket before fixture timeout",
        )
        fixture.require(
            malicious.attack_at is not None and closed_at - malicious.attack_at < 5,
            "malformed WebSocket did not fail promptly",
        )
        fixture.require(
            malicious.connections == 1,
            "client reconnected or replayed WebSocket establishment",
        )
        if case in HANDSHAKE_CASES:
            fixture.require(
                malicious.binary_sent == 0,
                "handshake rejection depended on binary data",
            )
        ...
        fixture.require(
            stats.get("startup_completed") == 1,
            "WebSocket opened without completed startup",
        )
        fixture.require(
            stats["requests"].get("POST /api/sync") == 20
            and sum(
                value
                for name, value in stats["requests"].items()
                if name.startswith("GET /api/") or name.startswith("GET /media/")
            )
            == 20,
            "client resumed HTTP carriers after WebSocket failure",
        )
        fixture.require(
            sum(request.get("uri") == "/api/realtime" for request in requests) == 1
            and sum(request.get("uri") == "/" for request in requests) == 1
            and not any(request.get("method") == "CONNECT" for request in requests),
        ...
        result = {
            ...
            "handshake_only_rejection": case in HANDSHAKE_CASES,
            "outer_connects": 0,
        }
...
def main():
    parser = argparse.ArgumentParser(
        description="Fail-closed native WebSocket checks behind trusted Caddy TLS."
    )
    ...
    parser.add_argument(
        "--listener", choices=("socks", "http", "both"), default="socks"
    )
    ...
    try:
        results = [
            run_case(args, run, protocol, case, listener)
            for protocol in ("h2",)
            for listener in (
                ("socks", "http") if args.listener == "both" else (args.listener,)
            )
