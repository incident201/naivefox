...
def fixture_module():
    path = Path(__file__).with_name("transport_fixture.py")
...
def reserve_ports(bind_address):
    ...
    try:
        ...
        return dict(
            zip(("socks", "http"), (sock.getsockname()[1] for sock in reservations))
        )
    finally:
...
def open_tunnel(fixture, address, port, kind, target_port):
    ...
    try:
        if kind == "socks":
            ...
            fixture.require(
                fixture.receive(connection, 2) == b"\x05\x00",
            ...
            while not response.endswith(b"\r\n\r\n"):
                ...
                fixture.require(
                    len(response) <= 16384, "HTTP CONNECT response too large"
                )
...
def exercise_address(fixture, ports, address, target_port):
    ...
    for kind in ("socks", "http"):
        with open_tunnel(
            fixture, address, ports[kind], kind, target_port
        ) as connection:
            ...
            while chunk := connection.recv(16384):
                ...
                fixture.require(
                    len(received) <= len(payload), "echo exceeded the sent payload"
                )
            fixture.require(
                received == payload, "listener transfer or half-close failed"
            )
...
def run_client(
    fixture, args, run, protocol, proxy_port, credentials, bind_address, target_port
):
    binding = "wildcard" if bind_address == "0.0.0.0" else "explicit-interface"
    directory = run / binding
    ...
    config = fixture.client_config(
        protocol, proxy_port, *credentials, ports, len(addresses) * 2
    )
    config["listen"] = [
        f"socks://{bind_address}:{ports['socks']}",
        f"http://{bind_address}:{ports['http']}",
    ]
    config_path = directory / "config.json"
    fixture.private_json(config_path, config)
    process = fixture.Process(
        [str(args.runtime), str(config_path)],
        directory,
        "client",
    ...
    try:
        ...
        def ready():
            ...
            return (
                f"SOCKS5 listening on {bind_address}:{ports['socks']}" in text
        ...
        for port in ports.values():
            output = subprocess.check_output(
                ["ss", "-H", "-ltn", f"sport = :{port}"], text=True
            )
            fixture.require(
                any(
                    line.split()[3] == f"{bind_address}:{port}"
                    for line in output.splitlines()
                ),
                "kernel listener address differs from config",
            )
    ...
    return {
        "protocol": protocol,
        "binding": binding,
        "nonloopback_connections": 2,
        "loopback_connections": 2 if binding == "wildcard" else 0,
        "payload_bytes_per_connection": 65536,
        "half_close": True,
    }
...
def main():
    ...
    parser.add_argument(
        "--address",
        required=True,
        help="assigned non-loopback IPv4 address of this Linux host",
    )
    parser.add_argument("--protocol", choices=("h2", "h3", "both"), default="both")
    args = parser.parse_args()
    if not sys.platform.startswith("linux"):
        parser.error(
            "this gate runs a native Linux client; it does not claim Windows or Android coverage"
        )
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as probe:
        probe.bind((
            args.address,
            0,
        ))  # Fail before starting processes if the address is not assigned.
    ...
    try:
        ...
        fixture.private_json(
            result, {"status": "PASS", "platform": "linux", "cases": results}
        )
        success = True
