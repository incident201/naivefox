...
SPEC = importlib.util.spec_from_file_location(
    "hybrid_matrix", HERE / "carrier_capture.py"
)
...
class HybridMatrixTests(unittest.TestCase):
    def test_observer_merges_both_transport_families_and_distinguishes_ids(self):
        tcp = [
            {"time": 0.1, "frame": 2, "flow": "0", "wire_size": 60, "direction": 1},
            {"time": 0.4, "frame": 4, "flow": "0", "wire_size": 1500, "direction": -1},
        ]
        quic = [
            {"time": 0.05, "frame": 1, "flow": "0", "wire_size": 1280, "direction": 1},
            {"time": 0.2, "frame": 3, "flow": "0", "wire_size": 1200, "direction": -1},
        ]
        events = MATRIX.merge_outer_events(tcp, quic)
        self.assertEqual([item["frame"] for item in events], [1, 2, 3, 4])
        self.assertEqual(
            MATRIX.wire_summary(events),
            {
                "wire_bytes": 4040,
                "packets": 4,
                "client_wire_bytes": 1340,
                "server_wire_bytes": 2700,
                "outer_flows": 2,
            },
        )
    ...
    def test_coalesced_quic_ids_do_not_create_a_phantom_connection(self):
        quic = [
            {"time": 0.1, "frame": 1, "flow": "0;0", "wire_size": 1280, "direction": 1},
            {"time": 0.2, "frame": 2, "flow": "0", "wire_size": 1200, "direction": -1},
        ]
        self.assertEqual(
            MATRIX.wire_summary(MATRIX.merge_outer_events([], quic))["outer_flows"], 1
        )
    ...
    def test_passive_observer_preserves_scenario_and_late_websocket_traffic(self):
        ...
        tcp = [event(0.3, 3, "0", 60, 1, True), event(0.4, 4, "0", 1500, -1)]
        quic = [
            event(0.1, 1, "0", 1280, 1, packet_types=("0",)),
            event(0.2, 2, "0", 1200, -1),
        ]
        merged = MATRIX.merge_outer_events(tcp, quic)
        row = {
            "scenario": "matched_application",
            "label": "naivefox",
            "naivefox_arm": "native-socks",
            "experiment_block": "block-0",
        }
        with mock.patch.object(
            MATRIX, "outer_events", return_value=(merged, tcp, quic, [])
        ), mock.patch.object(MATRIX.features, "extract_handshake"), mock.patch.object(
            MATRIX.features, "extract_transport_parameters"
        ), mock.patch.object(MATRIX.features, "add_h2_features"), mock.patch.object(
            MATRIX.features, "add_h3_features"
        ):
            document, wire = MATRIX.passive_document(
                Path("unused.pcapng"), 443, "h3", row, "sample"
            )
    ...
    def test_late_websocket_cannot_change_early_views(self):
        analysis = MATRIX.module(
            "hybrid_invariance_analysis", "analyze-camouflage-arms.py"
        )
        syn_row = dict.fromkeys(
            (
                "tcp.options.mss_val",
        ...
        def event(index, flow, syn=False):
            return {
                ...
                "out_of_order": False,
                "lost_segment": False,
                "packet_types": ["0"] if index == 1 else [],
                "versions": ["0x00000001"],
                "dcil": ["8"],
                "scil": ["8"],
                "row": syn_row,
        ...
        for protocol in ("h2", "h3"):
            ...
            for variation in (1, 2):
                ...
                tcp, quic = (
                    (startup + late, []) if protocol == "h2" else (late, startup)
                )
                records = (
                    [
                        {
                ...
                records += [
                    {
                        ...
                        "frame": 41,
                        "direction": -1,
                        "length": 100 * variation,
                    }
                ] * variation

                def handshake(pcap, family, port, output, flow_filter=None):
                    start_filter = (
                        "tcp.stream" if protocol == "h2" else "quic.connection.number"
                    ) + "==0"
                    stable = family == protocol and flow_filter == start_filter
                    output.update({
                        "tls_client_hello_count": 1 if stable else variation,
                        "tls_cipher_stable"
                        if stable
                        else f"tls_cipher_late_{variation}": 1,
                    })
                ...
                with mock.patch.object(
                    ...
                    "outer_events",
                    return_value=(
                        MATRIX.merge_outer_events(tcp, quic),
                        tcp,
                        quic,
                        records,
                    ),
                ), mock.patch.object(
                    MATRIX.features, "extract_handshake", side_effect=handshake
                ), mock.patch.object(MATRIX.features, "extract_transport_parameters"):
                    document, _ = MATRIX.passive_document(
                        Path("unused"), 443, protocol, row, "sample"
                    )
                documents.append(document["features"])
            names = sorted(set(documents[0]) | set(documents[1]))
            self.assertTrue(
                all(
                    name.startswith(analysis.ANALYSIS.FEATURE_PREFIXES)
                    for name in names
                )
            )
            for view in ("initial_packets_16", "packets_17_32"):
                ...
                self.assertEqual(
                    {name: documents[0].get(name, 0) for name in selected},
                    {name: documents[1].get(name, 0) for name in selected},
                    (protocol, view),
                )
    ...
    def test_strict_windows_ignore_future_startup_records_and_quic_phases(self):
        first = {
            f"packet_{index:03d}_wire_size_signed": index for index in range(1, 49)
        }
        first.update({
            "initial_16_packet_count": 16,
            "initial_32_packet_count": 32,
            "initial_250ms_packet_count": 32,
            "tls_record_001_signed_length": 100,
            "tls_record_017_signed_length": 200,
            "tls_client_hello_count": 1,
            "quic_phase_position_17_c_handshake": 1,
            "quic_phase_position_24_s_short": 1,
        })
        second = dict(first)
        second.update({
            "tls_record_001_signed_length": 1500,
            "tls_record_017_signed_length": 1200,
            "quic_phase_position_17_c_handshake": 0,
            "quic_phase_position_24_s_short": 0,
            "tls_client_hello_count": 2,
            "packet_040_wire_size_signed": 500,
        })
        for view in MATRIX.VIEWS[:-1]:
            names = MATRIX.matrix_view_feature_names(first, view)
            self.assertTrue(names)
            self.assertEqual(
                {name: first[name] for name in names},
                {name: second[name] for name in names},
            )
        self.assertNotEqual(first, second)

    @unittest.skipUnless(
        os.environ.get("NAIVEFOX_CAPTURE_ISOLATED_NETWORK_ENTERED") == "1"
        and os.environ.get("NAIVEFOX_CAPTURE_PACKET_TEST_ROOT"),
        "requires isolated capture fixture",
    )
    def test_actual_capture_canary_never_enters_origin_features(self):
        ...
        with tempfile.TemporaryDirectory(
            prefix="canary-", dir=root
        ) as name, socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as receiver:
            ...
            try:
                ...
                capture.stop()
            markers = MATRIX.features.tshark_rows(
                str(capture.pcap),
                [],
                f"udp.port=={capture.marker_port}",
                ["frame.number"],
            )
    ...
    def test_speed_loss_is_not_time_growth(self):
        ...
        self.assertLess(
            MATRIX.penalties(20, 10, 125, 100)["effective_rate_loss_percent"], 0
        )
    ...
    def test_existing_analyzer_accepts_complete_native_superblocks(self):
        ...
        schedule = analysis.SUPERBLOCKS.schedule_rows(
            7, "h2", 2, ["browser_page"], MATRIX.ARMS
        )
        names = [
            "packet_001_wire_size_signed",
            "packet_017_wire_size_signed",
            "whole_server_wire_bytes",
        ]
        rows = []
        for index, row in enumerate(schedule):
            value = (
                1
                if row["label"] == "firefox_a"
                else 2
                if row["label"] == "firefox_b"
                else 3
            )
            rows.append({
                **row,
                "session_id": f"sample-{index}",
                "features": dict.fromkeys(names, value),
            })
        report = analysis.build_report(
            SimpleNamespace(
                ...
                permutations=99,
                min_blocks=30,
                views=("initial_packets_16", "packets_17_32", "whole"),
            ),
            rows,
            names,
        )
        ...
        self.assertEqual(
            set(report["protocols"]["h2"]["views"]["whole"]["arms"]), set(MATRIX.ARMS)
        )
