...
class NaiveFoxTransportCodec : public ::testing::Test {
 protected:
...
bool NaiveFoxTransportCodec::sOwnsNSS = false;
...
TEST_F(NaiveFoxTransportCodec, IndependentWireFixture) {
  const std::vector<Frame> frames{
      {Kind::Data, 0x01020304, 0xaabbccdd, {0x00, 0x7f, 0xff}}};
  const std::vector<uint8_t> fixture{
      'N',  'F',  'O',  'X',  0x89, 0xab, 0xcd, 0xef, 0x00, 0x00, 0x00, 0x23,
...
TEST_F(NaiveFoxTransportCodec, EmptyMaximumAndFrameCountBoundaries) {
  std::vector<uint8_t> encoded;
  std::vector<Frame> decoded;
...
TEST_F(NaiveFoxTransportCodec, AllKindsAndPadding) {
...
TEST_F(NaiveFoxTransportCodec, ReadyWebSocketCapacityBoundsFiller) {
...
TEST_F(NaiveFoxTransportCodec, WebSocketAckIsNotAStreamMessage) {
...
TEST_F(NaiveFoxTransportCodec, DirectionalWebSocketCapacities) {
...
TEST_F(NaiveFoxTransportCodec, EmptyAndPartFilledProfileCapacities) {
...
TEST_F(NaiveFoxTransportCodec, EveryTruncationAndExcessByteRejected) {
  std::vector<uint8_t> encoded;
...
TEST_F(NaiveFoxTransportCodec, CorruptHeadersNeverPublishPartialFrames) {
  std::vector<uint8_t> encoded;
  ASSERT_TRUE(Encode(
...
TEST_F(NaiveFoxTransportCodec, SendCreditTracksBytesAndRejectsInflation) {
  StreamState stream(3);
  std::vector<uint8_t> bytes(kMaxCell / 2, 0x7f);
  Frame frame;
  for (uint32_t i = 0; i < kReceiveWindow / (kMaxCell / 2); ++i) {
...
TEST_F(NaiveFoxTransportCodec, ReceiveCreditRequiresActualConsumerDelivery) {
  StreamState stream(4);
  ASSERT_TRUE(stream.Receive({Kind::Opened, 4, 0, {}}));
  Frame data{Kind::Data, 4, 0, std::vector<uint8_t>(kMaxCell / 2, 0x41)};
  for (uint32_t i = 0; i < kReceiveWindow / (kMaxCell / 2); ++i) {
...
TEST_F(NaiveFoxTransportCodec, Http3CreditIsBoundedAndFollowsDelivery) {
  const uint32_t window = 1024 * 1024;
  StreamState stream(7, true);
  ASSERT_TRUE(stream.Receive({Kind::Opened, 7, 0, {}}));
  EXPECT_EQ(stream.SendCredit(), window);
  EXPECT_EQ(stream.ReceiveCredit(), window);
  Frame data{Kind::Data, 7, 0, std::vector<uint8_t>(16384, 0x5a)};
  Frame sent;
  for (uint32_t offset = 0; offset < window; offset += data.body.size()) {
    data.sequence = offset;
    ASSERT_TRUE(stream.Receive(data));
    ASSERT_TRUE(stream.MakeData(data.body.data(), data.body.size(), sent));
  }
  EXPECT_EQ(stream.ReceiveCredit(), 0U);
  EXPECT_EQ(stream.SendCredit(), 0U);
  EXPECT_EQ(stream.BufferedBytes(), window);
  Frame credit;
  EXPECT_FALSE(stream.TakeCredit(credit));
  ASSERT_TRUE(stream.Delivered(123));
  EXPECT_EQ(stream.ReceiveCredit(), 0U);
  ASSERT_TRUE(stream.TakeCredit(credit));
  EXPECT_EQ(credit.body, Credit(7, 123).body);
  EXPECT_EQ(stream.ReceiveCredit(), 123U);
  EXPECT_EQ(stream.BufferedBytes(), window - 123);
  ASSERT_TRUE(stream.Receive(Credit(7, window)));
  EXPECT_FALSE(stream.Receive(Credit(7, 1)));
}

TEST_F(NaiveFoxTransportCodec, UncreditedDataFailsClosed) {
  StreamState stream(1);
  ASSERT_TRUE(stream.Receive({Kind::Opened, 1, 0, {}}));
  Frame data{Kind::Data, 1, 0, std::vector<uint8_t>(kMaxCell / 2, 0x41)};
  for (uint32_t i = 0; i < kReceiveWindow / (kMaxCell / 2); ++i) {
    data.sequence = i * data.body.size();
    ASSERT_TRUE(stream.Receive(data));
  }
...
TEST_F(NaiveFoxTransportCodec, StreamControlAndSequenceValidation) {
...
TEST_F(NaiveFoxTransportCodec, ResetIsTerminalAndMayRejectOpen) {
  StreamState stream(1);
...
TEST_F(NaiveFoxTransportCodec, ReceiveSequenceWrapsWithExactDataAndFinOffsets) {
  StreamState stream(1);
  ASSERT_TRUE(stream.Receive({Kind::Opened, 1, 0, {}}));
...
TEST_F(NaiveFoxTransportCodec, SendSequenceWrapsWithoutResettingCreditOrFin) {
  StreamState stream(1);
...
TEST_F(NaiveFoxTransportCodec, ReceiveWindowRemainsBoundedAcrossSequenceWrap) {
  StreamState stream(1);
  ASSERT_TRUE(stream.Receive({Kind::Opened, 1, 0, {}}));
  Frame data{Kind::Data, 1, 0, std::vector<uint8_t>(kMaxCell / 2, 0x41)};
  const uint64_t boundary = uint64_t{1} << 32;
  ...
  for (unsigned i = 0; i < kReceiveWindow / (kMaxCell / 2); ++i) {
    data.sequence = static_cast<uint32_t>(received);
    ASSERT_TRUE(stream.Receive(data));
    received += data.body.size();
  }
  EXPECT_GT(received, boundary);
  EXPECT_EQ(stream.ReceiveSequence(), kReceiveWindow - data.body.size());
  EXPECT_EQ(stream.ReceiveCredit(), 0U);
  EXPECT_EQ(stream.BufferedBytes(), kReceiveWindow);
  EXPECT_FALSE(
...
TEST_F(NaiveFoxTransportCodec, UploadRingWrapAndBound) {
  UploadBuffer buffer;
...
namespace mozilla::naivefox::wire {
namespace {
TEST_F(NaiveFoxTransportCodec, Http3CellStreamArbitraryBoundaries) {
  using namespace mozilla::naivefox::wire;
  std::vector<uint8_t> stream;
  for (size_t capacity : {size_t(512), size_t(8192), size_t(65536)}) {
    stream.push_back(uint8_t(capacity >> 24));
    stream.push_back(uint8_t(capacity >> 16));
    stream.push_back(uint8_t(capacity >> 8));
    stream.push_back(uint8_t(capacity));
    stream.insert(stream.end(), capacity, uint8_t(capacity / 512));
  }
  for (size_t chunk : {size_t(1), size_t(3), size_t(4095), size_t(65536)}) {
    CellStreamDecoder decoder;
    size_t cells = 0;
    for (size_t offset = 0; offset < stream.size();) {
      const size_t count = std::min(chunk, stream.size() - offset);
      ASSERT_TRUE(decoder.Feed(
          stream.data() + offset, count, [&](const std::vector<uint8_t>& body) {
            ++cells;
            return std::all_of(body.begin(), body.end(), [&](uint8_t b) {
              return b == uint8_t(body.size() / 512);
            });
          }));
      offset += count;
    }
...
TEST_F(NaiveFoxTransportCodec, Http3RejectsInvalidLengthAndDetectsTruncation) {
  using namespace mozilla::naivefox::wire;
  auto accept = [](const std::vector<uint8_t>&) { return true; };
  for (std::vector<uint8_t> prefix : {std::vector<uint8_t>{0, 0, 0, 0},
                                      {0, 0, 16, 0},
                                      {255, 255, 255, 255}}) {
    CellStreamDecoder decoder;
    EXPECT_FALSE(decoder.Feed(prefix.data(), prefix.size(), accept));
  }
  CellStreamDecoder decoder;
  const uint8_t partial[] = {0, 0, 2, 0, 42};
  EXPECT_TRUE(decoder.Feed(partial, sizeof(partial), accept));
