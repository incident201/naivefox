 ...
 public:
  ...
  nsresult Parse(Config& aConfig) {
    ...
    while (true) {
      ...
      } else if (key.EqualsLiteral("diagnostic-h2-finite-budgeted-downloads")) {
        if (sawDiagnosticH2FiniteBudgetedDownloads) {
          return Error(
              "duplicate diagnostic-h2-finite-budgeted-downloads field");
        }
        sawDiagnosticH2FiniteBudgetedDownloads = true;
        MOZ_TRY(ParseBoolean(
    ...
    if (parsed.mDiagnosticH2FiniteBudgetedDownloads &&
        !parsed.mDiagnosticH2FiniteStreamUploads) {
      return Error(
          "budgeted finite downloads require bidirectional read-through");
    }
