#!/usr/bin/env python3
"""Exercise HTML-derived no-connect startup against the actual Caddy binary."""
import argparse
import collections
import copy
...
spec = importlib.util.spec_from_file_location("site_fixture", Path(__file__).with_name("run-no-connect-tests.py"))
...
def write_site(root, case):
    ...
    (root / "css/main.css").write_text("@import url('/not-requested.css'); body{background:url('/large.png')}")
    html += "<link rel=stylesheet href=/css/main.css>"
    paths.append("/css/main.css")
    (root / "pics/icon.svg").write_text("<svg xmlns='http://www.w3.org/2000/svg'></svg>")
    html += "<link rel=icon href=/pics/icon.svg><link rel=preload as=image href='/pics/icon.svg#one'>"
    paths.append("/pics/icon.svg")
    html += "</head><body><template><img src=/inert.png></template><img src=/pics/icon.svg>"
...
def run_case(args, base, protocol, case, supplied=None):
    ...
    if supplied:
        ...
        expected = ["/favicon.ico", "/assets/site.css", "/assets/css/world.css",
                    "/assets/image-1.svg", "/assets/image-2.svg", "/assets/image-3.svg",
                    "/assets/image-4.svg", "/assets/app.js", "/assets/js/site.js"]
    else:
    ...
    try:
        ...
        caddy, port = fixture.start_caddy(inputs, run, protocol, target.server_address[1], user, password)
        ...
        for iteration in range(2):
            client, ports = fixture.start_client(inputs, run, "client-" + str(iteration),
                                                protocol, port, "no-connect", user, password, 2)
            processes.append(client)
            fixture.download(ports, "socks", target.server_address[1], 32768)
            fixture.wait_until(lambda: "No-connect websocket ready startup=20" in
                               client.log_path.read_text(errors="replace"),
                               "site bootstrap did not reach WSS", client, timeout=60)
            fixture.download(ports, "http", target.server_address[1], 32768)
            client.exited_cleanly()
        caddy.stop()
        records = [json.loads(line) for line in (run / "access.jsonl").read_text().splitlines()]
        ...
        for path in expected:
            fixture.require(counts[("GET", path)] == 2, "selected resource was omitted or duplicated")
        allowed = {"/", *expected, "/api/sync", "/api/events/brief", "/api/events/state", "/api/realtime"}
        allowed.update("/media/chunk/" + str(i) for i in range(6, 18))
        fixture.require(all(item["uri"] in allowed and item["method"] != "CONNECT" for item in requests),
        ...
        fixture.require(stats.get("startup_completed") == 2 and stats.get("ws_opened") == 2,
                        "wrong complete HTTP/WS lifecycle")
        fixture.require(not target.failures and target.accepted_connections == 4,
                        "target bytes or stream lifecycle failed")
        result = {"protocol": protocol, "case": case, "resources": len(expected), "cold_carriers": 2,
                  "body_bytes_per_carrier": sum((inputs.application_root / ("index.html" if p == "/" else urlsplit(p).path[1:])).stat().st_size
                                               for p in ["/", *expected]),
                  "cache_disabled": True, "recursive_loads": 0, "status": "PASS"}
        fixture.private_json(run / "result.json", result)
        print("PASS", protocol, case, "resources", len(expected), "two cold starts, both listeners", flush=True)
        return result
    finally:
...
def main():
    ...
    parser.add_argument("--astral-site", type=Path, help="Optional private, extracted Astral Oath reference")
    ...
    parent = (args.work_dir or args.objdir / "no-connect").resolve()
    fixture.require(parent.is_relative_to(args.objdir), "work must remain below the object directory")
    ...
    try:
        ...
        for protocol in (("h2", "h3") if args.protocol == "both" else (args.protocol,)):
            ...
            if args.astral_site:
                results.append(run_case(args, run, protocol, "astral", args.astral_site.resolve(strict=True)))
