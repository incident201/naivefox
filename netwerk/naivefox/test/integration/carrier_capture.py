#!/usr/bin/env python3
"""Native NaiveFox screen; all encrypted origin flows are one observer unit."""
...
native = module("hybrid_native_fixture", "transport_fixture.py")
...
def merge_outer_events(tcp, quic):
    ...
    for family, items in (("tcp", tcp), ("quic", quic)):
        for item in items:
            ...
            event["flow"] = ";".join(
                family + ":" + value
                for value in dict.fromkeys(features.split_values(event["flow"]))
            )
            result.append(event)
...
def wire_summary(events):
    return {
        ...
        "packets": len(events),
        "client_wire_bytes": sum(
            event["wire_size"] for event in events if event["direction"] > 0
        ),
        "server_wire_bytes": sum(
            event["wire_size"] for event in events if event["direction"] < 0
        ),
        "outer_flows": len({
            value for event in events for value in features.split_values(event["flow"])
        }),
    }
...
def passive_document(pcap, port, protocol, row, sample):
    ...
    require(
        events and all(event["wire_size"] <= 1500 for event in events),
        "missing packets or offload superframes",
    )
    ...
    if protocol == "h2":
        ...
        require(
            quic
            and any(
                event["direction"] > 0 and "0" in event["packet_types"]
                for event in quic
            ),
            "missing captured H3 client Initial",
        )
    startup = tcp if protocol == "h2" else quic
    startup_ids = list(
        dict.fromkeys(
            value for event in startup for value in features.split_values(event["flow"])
        )
    )
    require(
        startup_ids and all(value.isdecimal() for value in startup_ids),
        "invalid startup flow identifiers",
    )
    require(
        protocol != "h3" or len(startup_ids) == 1,
        "more than one startup QUIC connection",
    )
    ...
    values = {}
    features.extract_handshake(
        str(pcap), protocol, port, values, flow_filter=flow_filter
    )
    all_handshakes = {}
    ...
    for index, identity in enumerate(dict.fromkeys(event["flow"] for event in tcp), 1):
        ...
        supplemental = {}
        features.extract_handshake(
            str(pcap), "h2", port, supplemental, flow_filter="tcp.stream==" + identity
        )
        values.update({
            f"whole_tcp_connection_{index:03d}_" + name: value
    ...
    if protocol == "h3":
        values.update({
            ("whole_" + name if name.startswith("tcp_syn_") else name): value
            for name, value in tcp_values.items()
        })
    else:
        values.update(tcp_values)
        values["whole_tcp_syn_count"] = values["tcp_syn_count"]
        values["tcp_syn_count"] = float(
            sum(
                event["direction"] > 0 and event["syn"] and not event["ack"]
                for event in tcp
                if event["flow"] == startup_id
            )
        )
    record_values = {}
    ...
    if protocol == "h2":
        features.add_tls_record_features(
            values, [record for record in records if record["flow"] == startup_id]
        )
    ...
    values["lifecycle_connection_count"] = float(
        len({
            value for event in events for value in features.split_values(event["flow"])
        })
    )
    values["lifecycle_reconnect_count"] = float(
        max(0, values["lifecycle_connection_count"] - 1)
    )
...
def verify_reference(proof_path, firefox, expected_base):
    ...
    require(
        proof.get("schema_version") == 1 and proof.get("git_base") == expected_base,
        "reference proof has another Git base",
    )
    task = proof.get("task_id", "")
    require(
        proof.get("task_url")
        == f"https://firefox-ci-tc.services.mozilla.com/api/queue/v1/task/{task}",
        "reference proof source is not the official task API",
    )
    for revision in (expected_base, proof.get("hg_revision")):
        require(
            f"index.gecko.v2.mozilla-central.revision.{revision}.firefox.linux64-opt"
            in proof.get("revision_routes", []),
            "reference proof lacks a revision route",
        )
    ...
    require(
        ...
        "reference proof lacks essential runtime files",
    )
    runtime = firefox.parent
    for name, expected in files.items():
        path = (runtime / name).resolve(strict=True)
        require(
            path.is_relative_to(runtime) and digest(path) == expected,
            "Firefox runtime differs from the verified official artifact",
        )
    require(
        digest(firefox) == files["firefox"],
        "selected Firefox executable is not the verified runtime",
    )
    return proof
...
def penalties(baseline_time, candidate_time, baseline_wire, candidate_wire):
    require(
        baseline_time > 0 and candidate_time > 0 and baseline_wire > 0,
        "invalid comparison denominator",
    )
    return {
...
class Capture:
    def __init__(self, directory, name, port):
        ...
        self.process = subprocess.Popen(
            [
                ...
                "any",
                "-f",
                f"port {port} or udp port {self.marker_port}",
                "-a",
                "duration:120",
                "-w",
                "-",
    ...
    def observe_nonce(self):
        ...
        while time.monotonic() < deadline:
            require(
                self.process.poll() is None, "capture exited before nonce observation"
            )
