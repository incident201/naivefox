...
"""Run NaiveFox and embedded-lifecycle gates on an ARM64 runtime."""
...
spec = importlib.util.spec_from_file_location(
    "transport_tests", HERE / "transport_fixture.py"
)
...
class AndroidFixture:
    def __init__(self, args, work):
        ...
        self.remote = f"/data/local/tmp/naivefox-native-{secrets.token_hex(8)}"
        self.ports = set()
        self.shared_configs = {}
        self.processes = []
        ...
        self.call(
            "push", str(args.package / "lib/arm64-v8a") + "/.", self.runtime + "/"
        )
        compiler = (
            args.ndk
        ...
        subprocess.run(
            [
                ...
                "-static-libstdc++",
                "-pthread",
                "-I" + str(args.package / "include"),
                str(HERE / "android_embedded_harness.cpp"),
                "-ldl",
                "-o",
                str(harness),
            ],
        ...
        subprocess.run(
            [
                ...
                "-pie",
                "-static-libstdc++",
                str(HERE / "android_transport_probe.cpp"),
                "-o",
                str(probe),
            ],
            check=True,
    ...
    def probe(
        ...
        ack = (
            (struct.pack("!Q", length) + suite.payload_digest(length)).hex()
            if operation == "upload"
            else "-"
        )
        ...
        suite.require(
            result.returncode == 0,
            f"Android {listener} {operation} failed ({result.returncode}); {log_path}",
        )

    def install_probes(self):
        suite.download = (
            lambda ports, listener, target_port, length=1024 * 1024, slow=False, host="localhost": (
                self.probe(ports, listener, target_port, "download", length, slow, host)
            )
        )
        suite.upload = (
            lambda ports, listener, target_port, length=1024 * 1024, host="localhost": (
                self.probe(ports, listener, target_port, "upload", length, host=host)
            )
        )
        suite.echo_wake = lambda ports, listener, target_port, idle_seconds=2: (
            self.probe(ports, listener, target_port, "idle", int(idle_seconds * 1000))
        )
        suite.response_first_transfer = lambda ports, listener, target_port, length=65536: self.probe(
            ports, listener, target_port, "response-first", length
        )
        suite.cancel_stream = lambda ports, target_port: self.probe(
            ports, "socks", target_port, "reset"
        )
        ...
        suite.reject_policy = lambda ports, listener, target_port, host="localhost": (
            self.probe(ports, listener, target_port, "policy-reject", host=host)
        )

        def reject(ports, listener, target_port, host="localhost", rejected=False):
            suite.require(
                rejected, "positive Android workloads must run in the native probe"
            )
    ...
    def call(self, *args, check=True):
        with (self.work / "adb.log").open("ab") as log:
            return subprocess.run(
                self.adb + list(args), stdout=log, stderr=log, check=check
            )
    ...
    def choose_listener_ports(self, args, directory, ports):
        ...
        if fixed is None:
            ...
            result = self.capture_command(
                output, "shell", self.remote + "/probe", "--allocate-listeners"
            )
            suite.require(
            ...
            suite.require(
                match is not None, "invalid Android guest listener allocation"
            )
            selected = dict(
                zip(("socks", "http"), (int(value) for value in match.groups()))
            )
        else:
            ...
            suite.require(
                dict(ports) == dict(fixed), "shared Android listener ports drifted"
            )
            selected = dict(fixed)
        suite.require(
            set(selected) == {"socks", "http"}
            and all(
                type(port) is int and 1 <= port <= 65535 for port in selected.values()
            )
            and selected["socks"] != selected["http"],
            "invalid Android listener port pair",
        )
        suite.require(
            not self.ports.intersection(selected.values()),
    ...
    def capture_failure(self, directory, remote, phase, exit_code, error=None):
        ...
        details = {
            "phase": phase,
            "adb_shell_exit_code": exit_code,
            "error_type": type(error).__name__ if error else None,
            "cause": "undetermined; inspect retained result and owned-process diagnostics",
        }
        pid_result = self.capture_command(
            diagnostics / "pid.txt", "shell", "cat", remote + "/pid"
        )
        result = self.capture_command(
            diagnostics / "result.txt", "shell", "cat", remote + "/result"
        )
        details["harness_result_available"] = (
            result is not None and result.returncode == 0
        )
        pid = (
            pid_result.stdout.strip()
            if pid_result is not None and pid_result.returncode == 0
            else b""
        )
        if pid.isdigit() and int(pid) > 0:
            ...
            self.capture_command(
                diagnostics / "process-status.txt",
                "shell",
                "cat",
    ...
    def start(self, args, directory, config, env, ports):
        ...
        config["listen"] = [
            f"{listener}://127.0.0.1:{port}" for listener, port in ports.items()
        ]
        config["host-resolver-rules"] = f"MAP localhost {self.args.host_alias}"
        config_path = Path(
            getattr(args, "client_config_path", directory / "config.json")
        )
        shared = getattr(args, "client_config_path", None) is not None
        if shared:
            label = str(config_path)
            if label not in self.shared_configs:
                self.shared_configs[label] = (
                    self.remote + "/shared-config-" + secrets.token_hex(6) + ".json"
                )
        ...
        if preserve_config and config_path.exists():
            suite.require(
                json.loads(config_path.read_bytes()) == config,
        ...
        class AndroidProcess(suite.Process):
            ...
            def capture_failure(self, phase, error=None):
                ...
                try:
                    fixture.capture_failure(
                        directory, remote, phase, self.process.poll(), error
                    )
                except (
                    OSError,
                    RuntimeError,
                    ...
                    try:
                        with (directory / "diagnostic-capture-error.log").open(
                            "a"
                        ) as log:
                            log.write(type(capture_error).__name__ + "\n")
                    except OSError:
                        print(
                            "Android failure diagnostics could not be written",
                            flush=True,
                        )
            ...
            def executed_config_bytes(self):
                result = subprocess.run(
                    fixture.adb + ["shell", "cat", remote_config],
                    capture_output=True,
                    timeout=10,
                    check=True,
                )
            ...
            def result_values(self):
                result = subprocess.run(
                    fixture.adb + ["shell", "cat", remote + "/result"],
                    text=True,
                    capture_output=True,
                    timeout=10,
                    check=True,
            ...
            def stop(self):
                ...
                if self.process.poll() is None:
                    ...
                    try:
                        ...
                        result = subprocess.run(
                            fixture.adb + ["shell", "cat", remote + "/pid"],
                            text=True,
                            capture_output=True,
                            check=True,
                        )
                        ...
                        suite.require(
                            pid.isdecimal(), "invalid owned Android harness PID"
                        )
            ...
            def exited_cleanly(self):
                try:
                    ...
                    suite.require(
                        "status=0" in result.stdout, "Android embedded result failed"
                    )
        ...
        process = AndroidProcess(
            self.adb + ["shell", command], directory, "client", dict(os.environ)
        )
        ...
        def ready():
            ...
            return all(
                f"listening on 127.0.0.1:{port}" in text for port in ports.values()
            )

        try:
            suite.wait_until(
                ready, "Android listeners did not start", process, timeout=60
            )
        except (OSError, RuntimeError, subprocess.SubprocessError) as error:
    ...
    def close(self):
        for process in reversed(self.processes):
            process.stop()
        suite.require(
            self.remote.startswith("/data/local/tmp/naivefox-native-"),
            "unsafe cleanup path",
        )
...
def main():
    ...
    work = Path(tempfile.mkdtemp(prefix="naivefox-android-", dir=root))
    fixture = None
    try:
