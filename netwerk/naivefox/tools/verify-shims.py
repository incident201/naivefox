...
def test_symbol_absence(topsrcdir, objdir):
    ...
    if not os.path.exists(libxul_path):
        print(
            f"FAIL: libxul.so not found in explicit object directory: {objdir}",
            file=sys.stderr,
        )
        return False
    ...
    try:
        nm_out = subprocess.check_output(
            ["nm", "-D", "-C", libxul_path], stderr=subprocess.DEVNULL, text=True
        )
    except Exception:
        nm_out = subprocess.check_output(
            ["readelf", "--wide", "--demangle", "-s", libxul_path],
    ...
    for desc, syms in forbidden_symbols:
        for s in syms:
            if s in nm_out:
                print(
                    f"  FAIL: Found forbidden symbol for {desc}: {s}", file=sys.stderr
                )
                failed = True
    if not failed:
        print(
            "  PASS: Zero heavy unwinder, Breakpad, JsonCPP, Abseil, or HarfBuzz symbols found."
        )
    return not failed
...
def test_profiler_stub_invariants(topsrcdir, objdir):
    ...
    comp_conf = os.path.join(
        topsrcdir, "netwerk", "naivefox", "core", "components.conf"
    )
    with open(comp_conf, "r", encoding="utf-8") as f:
        comp_text = f.read()
    assert "nsIProfiler" not in comp_text, (
        "nsIProfiler must not be registered in components.conf"
    )

    stub_path = os.path.join(
        topsrcdir, "netwerk", "naivefox", "core", "ProfilerNaiveFoxStub.cpp"
    )
    ...
    assert (
        "profiler_register_thread(const char* name, void* guessStackTop) {\n  return nullptr;\n}"
        in stub_text
    )
...
def test_security_manager_invariants(topsrcdir):
    ...
    assert (
        "NS_ERROR_NOT_AVAILABLE" in sec_text or "NS_ERROR_NOT_IMPLEMENTED" in sec_text
    )
    print(
        "  PASS: SystemPrincipal-only loads allowed; unprivileged/foreign loads fail-closed."
    )
    return True
...
def test_lean_dom_psm_invariants(topsrcdir):
    ...
    print(
        "  PASS: PSM app/PKCS7 verification returns NS_ERROR_NOT_IMPLEMENTED (fail-closed); OriginAttributes value semantics intact."
    )
    return True
...
def test_cache_crypto_boundary(topsrcdir):
    ...
    assert re.search(
        ...
        r"#ifdef MOZ_NAIVEFOX\s*return nullptr;\s*#else",
        crypto,
...
def test_rust_allocator_boundary(topsrcdir):
    ...
    assert "extern crate mozglue_static;" in product, (
        "GeckoAlloc must be linked, not just listed as an unused Cargo dependency"
    )
    assert 'naivefox_features.append("mozglue-static/moz_memory")' in build
    assert "NaiveFoxRustAllocatorSmoke" in product
    print(
        "  PASS: GeckoAlloc selected; project allocator gtest covers nested ThinVec/nsTArray ownership."
    )
    return True
...
def main():
    ...
    mode.add_argument(
        "--source-only", action="store_true", help="explicitly omit the binary check"
    )
    ...
    if not args.source_only and not objdir:
        parser.error(
            "provide --objdir/NAIVEFOX_OBJDIR, or use --source-only for Gate 1"
        )
