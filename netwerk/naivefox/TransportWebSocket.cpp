...
NS_IMPL_ISUPPORTS(TransportWebSocket, nsIWebSocketListener)
...
nsresult TransportWebSocket::Start(const TransportConfig& aConfig,
  ...
  MOZ_TRY(CreateOriginChannel(aConfig.mProxyUrl, aPath, ProxyProtocol::H2,
...
nsresult TransportWebSocket::Send(const nsACString& aMessage) {
  MOZ_ASSERT(NS_IsMainThread());
...
void TransportWebSocket::Close(nsresult aStatus) {
  MOZ_ASSERT(NS_IsMainThread());
  if (mClosing) {
    return;
  }
...
NS_IMETHODIMP TransportWebSocket::OnStart(nsISupports*) {
  MOZ_ASSERT(NS_IsMainThread());
...
NS_IMETHODIMP TransportWebSocket::OnStop(nsISupports*, nsresult aStatus) {
  MOZ_ASSERT(NS_IsMainThread());
  RefPtr<TransportWebSocket> self = this;
  mOpen = false;
  mClosing = true;
...
NS_IMETHODIMP TransportWebSocket::OnMessageAvailable(nsISupports*,
                                                     const nsACString&) {
  Close(NS_ERROR_ILLEGAL_VALUE);
  return NS_OK;
}

NS_IMETHODIMP TransportWebSocket::OnBinaryMessageAvailable(
...
NS_IMETHODIMP TransportWebSocket::OnAcknowledge(nsISupports*, uint32_t aSize) {
  MOZ_ASSERT(NS_IsMainThread());
  if (!mOpen || mClosing) {
    return NS_OK;
  }
  // Necko also acknowledges whole PING/PONG payloads (at most 125 bytes).
  // NFOX writes are at least 512 bytes and have a separate queue budget.
  if (aSize <= 125) {
    return NS_OK;
  }
  RefPtr<TransportWebSocket> self = this;
...
NS_IMETHODIMP TransportWebSocket::OnServerClose(nsISupports*, uint16_t aCode,
                                                const nsACString&) {
  Close(aCode == nsIWebSocketChannel::CLOSE_NORMAL ? NS_OK : NS_ERROR_ABORT);
  return NS_OK;
}

NS_IMETHODIMP TransportWebSocket::OnError() { return NS_OK; }
