...
struct ListenerConfig final {
  ...
  nsCString mHost, mUser, mPassword;
  uint16_t mPort = 0;
  bool mIPv6 = false;
};
...
nsresult ParseConfig(const nsACString&, Config&, nsACString&);
nsresult LoadConfigFile(const nsACString&, Config&, nsACString&);
nsresult ResolveAndCreateProfile(ProfileDirectory&, nsACString&);
}  // namespace mozilla::naivefox
#endif
