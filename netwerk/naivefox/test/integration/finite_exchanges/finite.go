...
func finiteSamePeer(a, b string) bool {
	first, firstErr := netip.ParseAddrPort(a)
	second, secondErr := netip.ParseAddrPort(b)
	// Native H2 pooling may migrate ordinary requests between connections.
	// Keep ownership bound to the authenticated peer address, not its TCP port.
	return firstErr == nil && secondErr == nil && first.Addr().Unmap() == second.Addr().Unmap()
}
...
func (h *Handler) newFinite(ctx context.Context, r *http.Request) (*finiteSession, error) {
	if _, err := netip.ParseAddrPort(r.RemoteAddr); err != nil {
		return nil, errors.New("invalid finite peer address")
	}
...
func (s *finiteSession) budgetedDownload(w http.ResponseWriter, r *http.Request, seq uint64) error {
	started := false
	err := s.down.ordered(r.Context(), s.done, seq, finiteDownloads, func() error {
		body := make([]byte, finiteBytes)
		remaining := finiteBytes
		for remaining > 0 {
			stop := context.AfterFunc(r.Context(), s.close)
			n, readErr := s.downIn.Read(body[:remaining])
			stop()
			if n > 0 {
				if !s.touch() {
					return io.ErrClosedPipe
	...
	if err != nil {
		s.close()
		if started {
			// A failed streamed body must not end as a valid shorter response.
...
func (h *Handler) serveFinite(w http.ResponseWriter, r *http.Request, ctx context.Context) error {
	...
	version := r.Header.Get("X-Naivefox-Finite")
	if r.ProtoMajor != 2 || (version != "1" && version != "2" && version != "3") {
		return bad()
	}
	...
	finiteRegistry.Unlock()
	if s == nil || s.version != version || s.handler != h || !finiteSamePeer(s.peer, r.RemoteAddr) ||
	...
	if op == "down" && s.version == "3" {
		return s.budgetedDownload(w, r, seq)
	}
